...
bool genplugin(endstone::CommandSender &sender, const endstone::Command &command, const std::vector<std::string> &args) {
  const std::string plugin_name = args[0];
    ...
    if(!std::filesystem::create_directories(removeTrailingSlash(plugin_dir))) { // the removeTrailingSlash wrapper should be removed after https://github.com/llvm/llvm-project/issues/60634 is fixed.
